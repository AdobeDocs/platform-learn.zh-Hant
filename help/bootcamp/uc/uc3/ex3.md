...
在上一個練習中，您建立了一個 **事件**. 您將其命名如下 `yourLastNameBeaconEntryEvent` 和已取代 `yourLastName` 加上您的姓氏。 這是建立事件的結果：
...
作為歷程的第二步，您需要新增 **推播** 動作。 前往熒幕左側，前往 **動作**，選取 **推播** 動作，然後將其拖放至歷程的第二個節點上。

![ACOP](./images/journeyactions.png)

在熒幕右側，您現在需要建立推播通知。

設定 **類別** 至 **行銷** 並選取可讓您傳送推播通知的推播表面。 在此情況下，要選取的推播表面為 **mmeeewis-app-mobile-bootcamp**.
...
您現在需要為欄位引進個人化權杖 **名字** ，儲存在 `profile.person.name.firstName`. 在左側功能表中，選取 **設定檔屬性**，向下捲動/導覽以尋找 **個人** 元素並按一下箭頭，可更深入瞭解，直到到達欄位為止 `profile.person.name.firstName`. 按一下 **+** 圖示以將欄位新增至畫布。 按一下&#x200B;**儲存**。
...
按一下事件的名稱，看起來應該像這樣： **yourLastNameBeaconEntryEvent**.
...
作為歷程的第三個步驟，您需要新增 **sendMessageToScreen** 動作。 前往熒幕左側，前往 **動作**，選取 **sendMessageToScreen** 動作，然後將其拖放至歷程中的第三個節點。 然後您會看到此內容。

![ACOP](./images/jomsg15.png)

此 **sendMessageToScreen** action是自訂動作，會將訊息發佈至店內顯示使用的端點。 此 **sendMessageToScreen** action需要定義許多變數。 您可以向下捲動直到顯示，以檢視這些變數 **動作引數**.

![ACOP](./images/jomsg16.png)

您現在需要設定每個動作引數的值。 請詳閱此表格以瞭解哪些值是必要的位置。
