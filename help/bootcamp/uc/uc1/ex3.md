...
在左側的功能表中，前往 **區段**. 您可以在此頁面上檢視所有現有區段的概觀。 按一下 **+建立區段** 按鈕以開始建立新區段。

![區段](./images/menuseg.png)

進入新的區段產生器後，您會立即注意到 **屬性** 功能表選項及 **XDM個別設定檔** 參考。
...
若要建置此區段，您必須新增體驗事件。 您可以按一下 **活動** 圖示於 **欄位** 功能表列。
...
選取 **名稱** 並拖放 **名稱** 物件，從左側選單移至區段產生器畫布中的 **活動** 區段。 然後您會看到以下內容：
...
每次將元素新增至區段產生器時，您都可以按一下 **重新整理預估值** 按鈕，以取得區段中母體的新估計值。
...
您現在將返回區段概觀頁面，其中會示範符合您區段資格的客戶設定檔預覽。
