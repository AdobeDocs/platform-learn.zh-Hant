...
您可以使用Xcode的原生Swift套件管理器新增個別套件，而不使用CocoaPods和Pod檔案（如[產生SDK安裝指示](./configure-tags.md#generate-sdk-install-instructions)中所述）。 Xcode專案已經為您新增了所有套件相依性。 Xcode **[!UICONTROL 封裝相依性]**&#x200B;畫面應該如下所示：
...
| [AEP Edge身分](https://github.com/adobe/aepsdk-edgeidentity-ios) | AEP Edge Identity行動擴充功能(`AEPEdgeIdentity`)可在使用Adobe Experience Platform SDK和Edge Network擴充功能時，從行動應用程式中處理使用者身分資料。 |
...
| [AEP使用者設定檔](https://github.com/adobe/aepsdk-userprofile-ios) | Adobe Experience Platform使用者設定檔行動擴充功能(`AEPUserProfile`)是管理Adobe Experience Platform SDK之使用者設定檔的擴充功能。 |
...
| [AEP保證](https://github.com/adobe/aepsdk-assurance-ios) | Assurance （也稱為專案Griffon）是全新創新的擴充功能(`AEPAssurance`)，可協助您檢查、證明、模擬及驗證如何在行動應用程式中收集資料或提供體驗。 此擴充功能可讓您的應用程式提供保證。 |
