---
title: 活動
description: 瞭解如何在行動應用程式中收集事件資料。
...
Edge Network擴充功能提供API，可將Experience事件傳送至Platform Edge Network。 體驗事件是包含符合XDM ExperienceEvent結構描述定義的資料的物件。 更簡單地說，這類連結可擷取使用者在您的行動應用程式中的動作。 Platform Edge Network收到資料後，即可將資料轉送至資料流中設定的應用程式和服務，例如Adobe Analytics和Experience Platform。 進一步瞭解 [體驗事件](https://developer.adobe.com/client-sdks/documentation/getting-started/track-events/) 產品檔案內。

## 先決條件

* 更新PodFile及必要的SDK。
* AppDelegate中註冊的擴充功能。
* 已設定MobileCore使用您的開發AppId。
* 匯入的SDK。
* 使用上述變更成功建置並執行應用程式。
...
Adobe Experience Platform Edge擴充功能可將遵循先前定義XDM結構描述的事件傳送至Adobe Experience Platform Edge Network。
...
1. 開始建構物件：
   ...
   * eventType：說明發生的事件，使用 [已知值](https://github.com/adobe/xdm/blob/master/docs/reference/classes/experienceevent.schema.md#xdmeventtype-known-values) 可能的話。
   * commerce.productViews.value：提供事件的數值。 如果是布林值(或Adobe Analytics中的「計數器」)，該值一律為1。 如果是數值或貨幣事件，值可以大於1。

1. 在您的結構描述中，識別與事件相關聯的任何其他資料。 在此範例中，包括 `productListItems` 這是用於商務相關事件的標準欄位集：
...
1. 開始建構物件。
   ...
   >  標準欄位群組一律以物件根目錄開頭。
   >
   >  自訂欄位群組一律以Experience Cloud組織獨有的物件開頭，在此範例中為「_techmarketingdemos」。
...
上述範例有望說明在建構XDM資料物件時的思維過程。 接下來，我們將在Luma應用程式中新增畫面檢視追蹤。
...
1. 對應用程式中的每個畫面重複此動作，正在更新 `stateName` 隨身攜帶。
...
1. 執行動作並尋找 `hitReceived` 事件來自 `com.adobe.edge.konductor` 廠商。
...
>[!NOTE]
>
>為清楚起見，所有值皆為硬式編碼。 在真實世界中，值會以動態方式填入。


### 在Luma應用程式中實作

您應該擁有所有工具，可以開始將資料收集新增至Luma範例應用程式。 以下是您可以遵循的假設性追蹤需求清單。
...
現在您已收集事件並傳送至Platform Edge Network，系統會將它們傳送至中設定的應用程式和服務。 [資料串流](create-datastream.md). 在稍後的課程中，您會將此資料對應至 [Adobe Analytics](analytics.md) 和 [Adobe Experience Platform](platform.md).
