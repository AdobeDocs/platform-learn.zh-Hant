...
* 我們必須在Luma網站上實作Web SDK，以將有關網站訪客活動的資料傳送至Experience Platform Edge網路。 我們將使用標籤（先前稱為Launch）進行簡單的實作
...
首先，我們將設定資料串流。 資料串流會告訴Experience Platform Edge Network在從Web SDK呼叫收到資料後要將資料傳送到何處。 例如，您要將資料傳送至Experience Platform、Adobe Analytics或Adobe Target嗎？ 資料串流在資料收集使用者介面（前身為Launch）中進行管理，對於透過Web SDK進行資料收集至關重要。
...
1. 請注意，我們如何看到在`web.webpagedetails.pageView`傳送事件[!UICONTROL 動作中指定的]事件型別，以及其他遵循`AEP Web SDK ExperienceEvent Mixin`格式的現成變數
...
1. 開啟[Luma網站](https://newluma.enablementadobe.com)
