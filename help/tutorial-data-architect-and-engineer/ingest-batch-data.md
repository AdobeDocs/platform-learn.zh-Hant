...
1. 從您的本機`luma-crm.json`資料夾中選取下載的`Luma Tutorial Assets`
...
最後，透過依照`Luma CRM Id`名稱空間查詢其中一個設定檔（例如`f660ab912ec121d1b1e928a0bb4bc61b`），確認已建立您的其中一個設定檔
...
再次在`5625458`名稱空間中查詢值為`loyaltyId`的設定檔，檢視其設定檔中是否有任何購買事件。 您應該會看到一次購買。 您可以選取&#x200B;**[!UICONTROL 檢視JSON]**，深入瞭解購買的詳細資料：
