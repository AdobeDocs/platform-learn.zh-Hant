...
1. 從`Luma Loyalty Id`身分識別名稱空間&#x200B;**[!UICONTROL 下拉式清單中選取]**&#x200B;名稱空間
1. 選取&#x200B;**[!UICONTROL 套用]**
1. 選取&#x200B;**[!UICONTROL 儲存]**
...
1. 在`Luma CRM Schema`中，使用`crmId`名稱空間將`Luma CRM Id`欄位標示為主要身分
1. 在`Luma Offline Purchase Events Schema`中，使用`loyaltyId`名稱空間將`Luma Loyalty Id`欄位標示為主要身分
1. 在`Luma Product Catalog Schema`中，使用`productSku`名稱空間將`Luma Product SKU`欄位標示為主要身分
...
透過我們選取的主要身分，可清楚瞭解`Luma Loyalty Schema`如何連線至`Luma Offline Purchase Events Schema`，因為它們都使用loyaltyId作為識別碼。 但CRM如何連線至離線購買事件？ 如何將離線購買與線上行為聯絡起來？ 我們如何分類隨產品目錄一起購買的產品？ 我們將使用其他身分欄位和結構描述關係。
...
1. 選取`crmId`欄位
1. 檢查&#x200B;**[!UICONTROL 身分]**&#x200B;方塊
1. 從`Luma CRM Id`身分識別名稱空間&#x200B;**[!UICONTROL 下拉式清單中選取]**&#x200B;名稱空間
