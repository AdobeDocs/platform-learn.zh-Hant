...
在本課程中，我們將建立身分識別名稱空間，並將身分識別欄位新增至結構描述。 完成此操作後，我們還能完成上一個課程的結構描述關係。
...
>[!NOTE]
>
>只有在您建立即時客戶設定檔時，才需要身分欄位。 如果您只將資料擷取到Data Lake，則不需要使用URL。
...
現在為Luma產品目錄結構描述設定另一個名稱空間，其中包含下列詳細資訊：
...
>[!NOTE]
>
>如果您偏好略過API練習，請隨時透過您使用的使用者介面方法來建立CRM名稱空間，並附上下列詳細資料：
>
...
讓我們將主要身分新增至 `Luma Loyalty Schema`：
...
1. 選取 `Luma Loyalty Id` 名稱空間來源 **[!UICONTROL 身分名稱空間]** 下拉式清單
1. 選取 **[!UICONTROL 套用]**
1. 選取 **[!UICONTROL 儲存]**
...
1. 在 `Luma CRM Schema`，加標籤 `crmId` 欄位作為主要身分識別，使用 `Luma CRM Id` 名稱空間
1. 在 `Luma Offline Purchase Events Schema`，加標籤 `loyaltyId` 欄位作為主要身分識別，使用 `Luma Loyalty Id` 名稱空間
1. 在 `Luma Product Catalog Schema`，加標籤 `productSku` 欄位作為主要身分識別，使用 `Luma Product SKU` 名稱空間

>[!NOTE]
>
>透過Web SDK收集的資料是架構中標籤身分欄位典型做法的例外情況。 Web SDK使用身分對應來標示身分 *於實作端* 因此，我們將決定 `Luma Web Events Schema` 在Luma網站上實作Web SDK時。 在稍後的課程中，我們會將Experience Cloud訪客ID (ECID)收集為主要ID，將crmId收集為次要ID。

透過我們選取的主要身分，我們可以清楚地瞭解 `Luma CRM Schema` 可以連線至 `Luma Offline Purchase Events Schema` 因為它們都使用 `loyaltyId` 作為識別碼。 但我們如何將離線購買與線上行為聯絡起來？ 我們如何分類隨產品目錄一起購買的產品？ 我們將使用其他身分欄位和結構描述關係。

<!--use a visual-->

### 為次要身分的XDM欄位加上標籤

可將多個身分欄位新增到結構描述。 非主要身分通常稱為次要身分。 若要將離線購買連結至線上行為，我們會將crmId作為次要識別碼新增至 `Luma Loyalty Schema` 以及稍後網路事件資料的內容。 讓我們更新 `Luma Loyalty Schema`：

1. 開啟 `Luma Loyalty Schema`
1. 選擇 `Luma Identity Profile Field group`
1. 選取 `crmId` 欄位
1. 檢查 **[!UICONTROL 身分]** 方塊
1. 選取 `Luma CRM Id` 名稱空間來源 **[!UICONTROL 身分名稱空間]** 下拉式清單
...
請注意，定義關係後，兩者都會指出該關係 **[!UICONTROL 組合]** 和 **[!UICONTROL 結構]** 結構描述編輯器的區段。
...
現在我們的身分已就緒，我們可以 [建立我們的資料集](create-datasets.md)！
