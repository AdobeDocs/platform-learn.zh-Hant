---
user-guide-title: 將您的行動應用程式從Adobe Target移轉至Offer Decisioning和Target擴充功能
user-guide-description: 為Target客戶提供的實作教學課程，說明如何成功將Mobile SDK實作從Target擴充功能移轉至Optimize擴充功能
solution: Target,Data Collection,Experience Platform
feature-set: Target,Experience Platform
feature: Implement Client-side,Implement Mobile,APIs/SDKs, Mobile SDK
role: Developer
level: Intermediate,Experienced
doc-type: tutorial
recommendations: catalog,noDisplay
jira: KT-17468
source-git-commit: 876e664a213aec954105bf2d5547baab5d8a84ea
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 3%

---


# 將您的行動應用程式從Adobe Target移轉至Offer Decisioning和Target擴充功能{#migrate-target-to-mobile-sdk-decisioning}

+ [教學課程簡介](overview.md)

+ [比較擴充功能](comparison.md)

+ [執行初始設定](initial-setup.md)

+ [取代SDK](replace-sdk.md)

+ [擷取活動](retrieve-activities.md)

+ [傳送引數](send-parameters.md)

+ [追蹤轉換事件](track-events.md)

+ [更新對象和個人資料指令碼](update-audiences.md)

+ [驗證實作](validate.md)

