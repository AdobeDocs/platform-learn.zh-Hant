---
title: 傳送引數 — 將行動應用程式中的Adobe Target實作移轉至Offer Decisioning和Target擴充功能
...
成功訂單後，購買引數會在訂單確認頁面上傳遞，並用於Target轉換和最佳化目標。 透過Offer Decisioning和Target擴充功能的Platform Mobile SDK實作，這些引數和會自動從作為`commerce`欄位群組一部分傳遞的XDM資料進行對應。
...
如果您未在`commerce`物件中傳遞`xdm`欄位，您可以使用`data.__adobe.target.orderId`、`data.__adobe.target.orderTotal`和`data.__adobe.target.productPurchasedId`欄位將訂單詳細資料傳遞至目標。
