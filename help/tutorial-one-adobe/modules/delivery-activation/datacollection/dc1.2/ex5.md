...
在本練習中，目標是使用Azure Blob儲存設定您的資料登陸區域Source聯結器。

Data Landing Zone是Adobe Experience Platform布建的Azure Blob儲存體介面，可讓您存取安全、雲端式的檔案儲存設施，將檔案帶入Platform。 Data Landing Zone支援SAS式驗證，且其資料受到標準Azure Blob儲存安全性機制的保護（無論在休息中還是在傳輸中）。 SAS式驗證可讓您透過公用網際網路連線，安全地存取您的資料登陸區域容器。
...
您現在會使用Azure命令列工具，透過AZCopy將資料內嵌至Adobe Experience Platform。
