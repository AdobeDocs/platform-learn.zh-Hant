...
連線完成後，您現在可以逐漸影響視覺效果。 Adobe Analytics和CJA的區別在於CJA需要資料檢視，才能在視覺化之前清除和準備資料。
...
對於&#x200B;**時區**，請選取時區&#x200B;**格林威治標準時間；蒙羅維亞、卡薩布蘭卡[GMT]**。 這是非常有趣的設定，因為有些公司在不同國家/地區和地區營運。 為每個國家分配正確的時區，可避免典型的資料錯誤，例如，相信秘魯大部分人在凌晨4:00購買T恤。

![示範](./images/ext7.png)

您也可以修改主要量度命名（「人員」、「工作階段」和「事件」）。 此為不必要操作，但有些客戶偏好使用人員、造訪和點選，而非人員、工作階段和事件(Customer Journey Analytics的預設命名慣例)。
...
真正重要的是&#x200B;**持續性設定**。 Evar和prop的概念在CJA中並不存在，但持續性設定可讓類似的行為發生。
