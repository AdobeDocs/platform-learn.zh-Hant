...
| 自訂事件 | 支援 | 部分支援透過 [監視掛接](https://github.com/adobe/alloy/wiki/Monitoring-Hooks) |
...
| at.js 自訂事件 | 不支援。不過， [回應token](https://experienceleague.adobe.com/docs/experience-platform/edge/personalization/adobe-target/accessing-response-tokens.html) 會公開為 `propositions` 回應 `sendEvent` 呼叫。 |
...
![at.js 2.0頁面載入時的行為](assets/target-at-js-2x-diagram.png)
