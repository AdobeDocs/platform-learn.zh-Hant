...
* 確保Target活動已正確傳送和呈現
* 檢查報表是否正常運作
* 重新造訪對象和設定檔指令碼，以確保它們與Platform Web SDK相容
...
| SPA元件重新呈現指定檢視時，`decisioning.propositionDisplay`事件不會引發 | 僅適用於SPA實作 |
| 在活動轉換之後會引發`decisioning.propsitionInteract`事件 | 從at.js `trackEvent`或`sendNotifications`移轉的「已點選元素」和自訂轉換應具有個別的事件呼叫 |
| 回應Token在`sendEvent`回應中傳回，並具有預期值 | 回應Token應可在Web SDK上正常運作 |
...
| VEC型活動會在SPA檢視變更上正確轉譯 | 僅適用於SPA實作 |
...
| 驗證專案 | 附註 |
|---|---|
| 針對Experience Cloud受眾返回的活動 | 例如，從Adobe Analytics發佈的區段 |
| 針對Experience Platform受眾返回的活動 | 僅適用於具有Experience Platform應用程式(例如RTCDP)的授權時 |
| 針對Audience Manager受眾返回的活動 | 例如，以造訪特定頁面為基礎的區段 |
...
執行上述驗證步驟後，您完全可以放心，Platform Web SDK實作已準備好投入生產。
