---
user-guide-title: 將Target從at.js 2.x移轉至Web SDK
breadcrumb-title: 將Target從at.js 2.x移轉至Web SDK
user-guide-description: Target客戶的實作教學課程，說明如何成功從at.js移轉至Web SDK實作。
feature-set: Target,Experience Platform
solution: Experience Platform,Data Collection,Target
feature: Implement Client-side,APIs/SDKs,at.js,AEP Web SDK, Web SDK,Implementation
role: Developer
level: Intermediate,Experienced
doc-type: tutorial
recommendations: catalog,noDisplay
hide: true
kt: 11159
source-git-commit: 43740912bc5a941aa21c5f38ed2c1aac74abffbc
workflow-type: tm+mt
source-wordcount: '89'
ht-degree: 5%

---


# 將Target at.js實作移轉至Web SDK {#migrate-target-to-websdk}

+ [教學課程簡介](introduction.md)

+ [移轉概述](migration-overview.md)

+ [規劃移轉](plan-migration.md)

+ [比較at.js與Platform Web SDK](detailed-comparison.md)

+ [執行初始設定](initial-setup.md)

+ [取代程式庫和預先隱藏方法](replace-library.md)

+ [呈現VEC型活動](render-vec-activities.md)

+ [轉譯表單式活動](render-form-based-activities.md)

+ [傳送參數](send-parameters.md)

+ [追蹤轉換事件](track-events.md)

+ [啟用跨網域支援](cross-domain.md)

+ [更新對象和設定檔指令碼](update-audiences.md)

+ [驗證實作](validate.md)

+ [除錯目標](debugging.md)

+ [預先擷取因應措施](prefetch-workaround.md)