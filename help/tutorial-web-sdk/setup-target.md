...
>[!NOTE]
>
>此教學課程使用[Luma網站](https://newluma.enablementadobe.com){target=_blank}，該網站已採用非同步實作標籤並具備閃爍緩解功能。 本節內容僅供瞭解閃爍緩解如何與Platform Web SDK搭配運作之用。
...
1. 選取&#x200B;**[!UICONTROL 編輯預先隱藏樣式]**&#x200B;按鈕
   ...
   ![同步實作的Target預先隱藏程式碼片段](assets/target-flicker-sync.png)
...
1. 開啟`all pages - adobeDataLayer push - send event - 50`規則
...
如果您偏好使用自訂程式碼對這些決定自行轉譯或執行動作，您可以保留[!UICONTROL 轉譯視覺個人化決定]設定停用。 Platform Web SDK具備彈性，提供這項功能供您完全控制。 您可以參考指南以取得有關[手動呈現個人化內容](https://experienceleague.adobe.com/zh-hant/docs/experience-platform/collection/use-cases/personalization/render-manual-propositions)的詳細資訊。
...
1. 導覽至Adobe Target介面
1. 使用活動URL的Luma首頁和您的Target工作區（如果有的話），建立體驗鎖定目標(XT)活動：
...
1. 移至[Luma示範網站](https://newluma.enablementadobe.com)，然後使用偵錯工具來[將網站上的標籤屬性切換為您自己的開發屬性](validate-with-debugger.md#use-the-experience-platform-debugger-to-map-to-your-tags-property)
1. 重新載入頁面
...
1. 新增條件以將規則限制在Luma首頁（不含查詢字串的路徑等於`/`或`/index.html`）。
...
1. 將下列程式碼貼到內容方塊中，以變更主圖中的call-to-action：
...
有些引數有助於從XDM物件對應至Target。 這些特殊的Target引數包括：
...
1. 開啟標籤規則`product detail pages - adobeDataLayer push - set product details variables - 20`
1. 新增新的&#x200B;**[!UICONTROL 動作]**
...
1. 選取&#x200B;**[!UICONTROL 目標]**&#x200B;物件
1. 將`entity.id`設為`Ecommerce Product Id`資料元素
1. 將`entity.name`設為`Ecommerce Product Name`資料元素
1. 將`user.categoryId`設為`Ecommerce Product Category`資料元素
1. 選取&#x200B;**[!UICONTROL 保留變更]**
1. 選取&#x200B;**[!UICONTROL 儲存]**
...
1. 開啟`all pages - adobeDataLayer push - send event - 50`規則
1. 選取`Adobe Experience Platform Web SDK - Send event`動作
1. 將`Data Variable`資料元素新增至[資料]欄位（如果尚未新增）
...
在Luma網站上，資料層會快速載入，並在傳送事件呼叫引發前完成定義。 這可讓我們使用單一呼叫，來擷取個人化內容(例如從Adobe Target)並傳送分析資料(例如傳送到Adobe Analytics)。
...
第二個通話將負責報告個人化內容的所有檢視並取代任何`decisioning.propositionDisplay`通話。
...
1. 建立名為`all pages - window loaded - send event - 50`的新規則
1. 將事件新增至規則。 使用&#x200B;**核心**&#x200B;延伸與&#x200B;**[!UICONTROL 視窗已載入]**&#x200B;事件型別
1. 將動作新增至規則。 使用&#x200B;**Adobe Experience Platform Web SDK**&#x200B;擴充功能和&#x200B;**傳送事件**&#x200B;動作型別
1. 選取&#x200B;**[!UICONTROL 使用引導式事件]**，然後選取&#x200B;**[!UICONTROL 收集分析]**
1. 這會將選取的&#x200B;**[!UICONTROL 包含演算後的主張]**&#x200B;核取方塊鎖定，以便傳送來自決策請求的已排入佇列的顯示通知。
1. 選取&#x200B;**[!UICONTROL 保留變更]**
1. **[!UICONTROL 儲存]**&#x200B;規則
1. 更新並重新建置程式庫
...
>[!TIP]
>
>如果您使用擷取決策主張的事件之後沒有Adobe Analytics事件，則會保留&#x200B;**[!UICONTROL 取消勾選「使用引導式事件」]**。 您需要手動選取所有選項，但是它會解除鎖定&#x200B;**[!UICONTROL 包含演算後主張]**&#x200B;的選項以及您的擷取請求。
...
1. 導覽至[Luma示範網站](https://newluma.enablementadobe.com)
1. 導覽至產品詳細資料頁面
