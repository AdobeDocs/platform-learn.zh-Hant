...
1. 開啟標籤規則`all pages - adobeDataLayer push - set global variables - 1`
1. 新增新的&#x200B;**[!UICONTROL 動作]**
...
1. 將`eVar1`設為`Page Name`資料元素
1. 設定`prop1`以複製`eVar1`的值
1. 若要示範`data`物件如何覆寫XDM對應值，請在&#x200B;**[!UICONTROL 其他屬性]**&#x200B;區段中，將頁面名稱設定為靜態值`test`
1. 選取&#x200B;**[!UICONTROL 保留變更]**
1. 選取&#x200B;**[!UICONTROL 儲存]**
...
1. 開啟標籤規則`all pages - adobeDataLayer push - send event - 50`
...
訪客進入特定頁面時，您可變更所使用的Adobe Analytics報表套裝。 這需要在資料串流和規則中進行設定。
...
1. 建立新規則，將其命名為`homepage - adobeDataLayer push - AA report suite override - 51`
...
1. 選取以開啟&#x200B;**[!UICONTROL 進階選項]**，輸入`51`。 這可確保規則在設定基準線XDM為`all pages - adobeDataLayer push - send event - 50`更新變數&#x200B;**[!UICONTROL 動作型別的]**&#x200B;之後執行。
1. 選取&#x200B;**[!UICONTROL 保留變更]**
...
1. 在&#x200B;**[!UICONTROL 路徑下]**&#x200B;設定`/` `OR` `/index.html`。 對於Luma示範網站，這可確保規則只在首頁上觸發
...
1. 以&#x200B;**[!UICONTROL XDM資料]**&#x200B;身分，選取您在`XDM Variable`建立資料元素[課程中建立的](create-data-elements.md)資料元素
...
1. 前往[Luma示範網站](https://newluma.enablementadobe.com){target="_blank"}
...
1. 在Luma網站上：
   ...
   >[!NOTE]
   >
   >由於您已登入，請花點時間驗證使用者`f660ab912ec121d1b1e928a0bb4bc61b`的驗證ID **`test@test.com`**，此驗證識別碼也已在`[!UICONTROL c.a.x.identitymap.lumacrmid.[0].id]`中擷取

### 報表套裝覆寫驗證

您在上方設定了[Luma首頁](https://newluma.enablementadobe.com)的資料流覆寫。  驗證此設定的方式
...
移至[Livingston All-Purpose Tight product page](https://newluma.enablementadobe.com/product.html?id=LLMP09)等產品頁面。  驗證Analytics是否擷取內容頁面檢視。
...
1. 另請注意`[!UICONTROL c.a.x.eventType]`已設為`commerce.productViews`，因為您位於產品頁面。
   ...
   >[!TIP]
   >
   > `product detail pages - adobeDataLayer push - set product details variables - 20`規則正在覆寫`eventType`規則設定的`all pages - adobeDataLayer push - set global variables - 1`的值，因為它設定為稍後在順序中觸發
...
1. 將[Marco輕量型主動式連帽上衣](https://newluma.enablementadobe.com/product.html?id=LLMH13)加入購物車
1. 前往[購物車頁面](https://newluma.enablementadobe.com/cart.html)，檢查Edge追蹤
...
>[!TIP]
>
> 完成本課程後，建議您停用`homepage - adobeDataLayer push - AA report suite override - 51`規則。
