---
title: 新增Adobe Target及標籤
description: 了解如何使用標籤搭配at.js、頁面載入請求、參數、訂單請求和自訂頁首/頁尾程式碼來實作Adobe Target。 本課程屬於「在網站中實作Experience Cloud」教學課程的一部分。
...
* 新增在將Target與非同步標籤內嵌程式碼搭配使用時，用於管理閃爍的預先隱藏程式碼片段
* 新增 Target v2 擴充功能
* 引發頁面載入請求 (先前稱為「全域 mbox」)
* 將參數新增至頁面載入請求
* 說明如何將設定檔和實體參數新增到頁面載入請求
* 使用必要的參數引發訂單確認請求
...
若要完成本節中的課程，您必須先完成 [設定標籤](create-a-property.md) 和 [新增Identity服務](id-service.md).

## 新增 Target 預先隱藏程式碼片段

開始之前，我們需要稍微更新標籤內嵌程式碼。 非同步載入標籤內嵌程式碼時，必須等到Target程式庫完全載入並執行其內容交換後，頁面才能完成轉譯。 這可能會導致所謂的閃爍問題，發生此問題時，會先短暫地顯示預設內容，然後再更換為 Target 指定的個人化內容。若要避免發生這種閃爍問題，強烈建議您在標籤的非同步內嵌程式碼之前，立即以硬式編碼撰寫特殊的預先隱藏程式碼片段。
...
開啟範例頁面，然後如下圖所示，將其貼到標籤內嵌程式碼的前面（不需擔心行號不同）。 在此螢幕擷取中，預先隱藏的程式碼片段已縮制：
...
Adobe Target 擴充功能支援將 Target 的 JavaScript SDK 用於現代網路 at.js 的用戶端實施。仍在使用Target舊版程式庫mbox.js、 [應升級至at.js 2.x](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/mbox-implement/migrate-mbox/target-atjs-implementation.html) 來使用標籤。
...
1. 前往 **[!UICONTROL 規則]** 在左側導覽列中，然後按一下 `All Pages - Library Loaded` 開啟規則編輯器
...
現在您已新增Target v2擴充功能並引發 `Load Target` 和 `Fire Page Load Request` 動作時，應會在使用標籤屬性的所有頁面上提出頁面載入請求。
...
在 Target 請求中傳遞參數可將強大的功能新增到目標鎖定、測試及個人化活動。標籤擴充功能提供可傳遞參數的兩個動作：
...
1. 前往 **[!UICONTROL 規則]** 在左側導覽列中，然後按一下 `All Pages - Library Loaded` 來開啟規則編輯器。
...
與請求參數類似，設定檔參數是透過 Target 請求傳遞。不過，設定檔參數會儲存在 Target 訪客設定檔資料庫中，且在[訪客的設定檔期間](https://experienceleague.adobe.com/docs/target/using/audiences/visitor-profiles/visitor-profile-lifetime.html)持續存在。您可以在網站的某個頁面上設定這些參數，並用於其他頁面上的 Target 活動。以下是來自某個汽車網站的範例。當訪客前往車輛頁面時，您可以傳遞設定檔參數「profile.lastViewed=sportscar」，以記錄其對於該特定車輛的興趣。當訪客瀏覽到其他非車輛的頁面時，您可以根據其檢視的上一個車輛鎖定目標內容。設定檔參數適用於很少變更或只會在某些頁面上使用的屬性

在本教學課程中，您不需要傳遞任何設定檔參數，但工作流程幾乎與剛剛傳遞 `pageName` 參數時的做法相同。有一項差異是，您必須為設定檔參數名稱加上 `profile.` 前置詞。這就是「userType」設定檔參數在 `Add Params to Page Load Request` 動作中看起來的樣子：

![配置設定檔案參數](images/target-profileParameter.png)

### 實體參數

實體參數是 [Recommendations 實施](https://experienceleague.adobe.com/docs/target/using/recommendations/plan-implement.html)中使用的特殊參數 ，主要原因有三：
...
在本教學課程中，您不需要傳遞任何實體參數，但工作流程與先前傳遞 `pageName` 要求參數 — 只要為參數加上前置詞為「entity」的名稱即可。 並將參數對應至相關的資料元素即可。請注意，某些通用實體已保留必須使用的名稱 (例如用於產品 SKU 的 entity.id)。以下是在 `Add Params to Page Load Request` 動作中設定實體參數看起來的樣子：
...
Target屬性類似於標籤屬性和Analytics報表套裝。 如果企業擁有多個品牌、網站和行銷團隊，可能會針對每個網站或行動應用程式使用不同的Target屬性、標籤屬性和Analytics報表套裝。 標籤屬性是以其內嵌程式碼區別、Analytics報表套裝是以其報表套裝ID區別，而Target屬性是以其屬性代號參數區別。


屬性代號必須使用自訂程式碼動作來實作，並搭配 `targetPageParams()` 函式。 如果您使用單一標籤屬性的不同at_property值，以不同的方式實作多個網站，您可以透過資料元素管理at_property值。
...
1. 複製 `<script></script>` 到剪貼簿

   ![從 Adobe Target 介面取得屬性代號](images/target-addATProperty-targetProperties.png)

1. 在標籤標籤中，前往 **[!UICONTROL 規則]** 在左側導覽列中，然後按一下 `All Pages - Library Loaded` 來開啟規則編輯器。
...
1. 檢查 **[!UICONTROL 全域執行]** 框 `targetPageParams()` 在全局範圍中聲明
1. 按一下&#x200B;**[!UICONTROL 保留變更]**
...
>[!WARNING]
>
>如果您嘗試新增 `at_property` 參數 **[!UICONTROL 將參數新增至頁面載入請求]** 動作時，參數會填入網路請求中，但Target的可視化體驗撰寫器(VEC)在載入頁面時無法自動偵測。 一律填入 `at_property` 使用 `targetPageParams()` 函式。
...
最佳實務是在所有訂單漏斗（甚至非零售網站）中使用訂單確認請求。 例如，潛在客戶產生網站通常會有潛在客戶漏斗，其尾端會產生不重複的「lead id」。這些網站應將靜態值 (例如「1」) 用於 orderTotal 以實施訂單請求。

如果使用不支援A4T的Automated Personalization活動，將Analytics for Target(A4T)整合用於其大部分報表的客戶也可能想要實施訂單請求。 此外，訂單請求是 Recommendations 實施中的關鍵元素，可根據購買行為支援演算法。如需A4T支援的最新資訊，請參閱 [檔案](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t.html?lang=en#section_F487896214BF4803AF78C552EF1669AA).
...
1. 按一下&#x200B;**[!UICONTROL 事件 > 新增]**
   ...
   1. 在 **[!UICONTROL 進階選項]**，變更 `Order` to `60` 這樣它就會在 `Load Target` 動作(位於 `All Pages - Library Loaded` 規則 `Order` 設為 `50`)
   1. 按一下&#x200B;**[!UICONTROL 保留變更]**
...
在某些罕見情況下，您可能需要提出頁面載入和訂單確認請求以外的Target請求。 例如，有時候您想要用於個人化的重要資料在標籤內嵌程式碼之前並未在頁面上定義，而是可能會在頁面底部以硬式編碼撰寫，或從非同步API請求傳回。 雖然將此要求用於內容傳送並非最佳方法（因為頁面已顯示），此資料可使用其他要求傳送至Target。 此資料可用來讓訪客設定檔變得更加豐富以供後續使用（使用設定檔參數），或用來填入Recommendations目錄。

在這些情況下，請使用核心擴充功能中的自訂程式碼動作，利用 [getOffer()](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/functions-overview/adobe-target-getoffer.html)/[applyOffer()](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/functions-overview/adobe-target-applyoffer.html) 和 [trackEvent()](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/functions-overview/adobe-target-trackevent.html) 方法引發 Launch 請求。這非常類似於您在[訂單確認請求](#order-confirmation-request)練習中所執行的操作，但您會使用不同的請求名稱，且不會使用特殊訂單參數。從自訂程式碼發出 Target 請求之前，請務必使用&#x200B;**[!UICONTROL 載入 Target]** 動作。

## 程式庫標題和程式庫頁尾

Target 使用者介面中的「編輯 at.js」畫面有可以貼上自訂 JavaScript 的位置，可使其在 at.js 檔案之前或之後立即執行。Library Header 有時可透過 [targetGlobalSettings()](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/functions-overview/targetgobalsettings.html) 函數用來覆寫 at.js 設定，或透過[資料提供者](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/use-data-providers-to-integrate-third-party-data.html)功能傳遞來自協力廠商的資料。Library Footer 有時用於新增 [at.js 自訂事件](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/functions-overview/atjs-custom-events.html)接聽程式。

若要在標籤中複製此功能，只需使用核心擴充功能中的「自訂程式碼」動作，並將動作排序在「載入Target」之前(Library Header)或之後(Library Footer)即可。 您可以使用與 `Load Target` 動作相同的規則執行此操作 (如下圖所示)，或使用具有事件或順序設定的個別規則，穩定地在包含 `Load Target` 的規則之前或之後引發：
