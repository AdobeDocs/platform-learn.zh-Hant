...
與請求參數類似，設定檔參數是透過 Target 請求傳遞。不過，設定檔參數會儲存在 Target 訪客設定檔資料庫中，且在[訪客的設定檔期間](https://experienceleague.adobe.com/docs/target/using/audiences/visitor-profiles/visitor-profile-lifetime.html?lang=zh-Hant)持續存在。您可以在網站的某個頁面上設定這些參數，並用於其他頁面上的 Target 活動。以下是來自某個汽車網站的範例。當訪客前往車輛頁面時，您可以傳遞設定檔參數「profile.lastViewed=sportscar」，以記錄其對於該特定車輛的興趣。當訪客瀏覽到其他非車輛的頁面時，您可以根據其檢視的上一個車輛鎖定目標內容。設定檔參數適用於很少變更或只會在某些頁面上使用的屬性
...
實體參數是 [Recommendations 實施](https://experienceleague.adobe.com/docs/target/using/recommendations/plan-implement.html?lang=zh-Hant)中使用的特殊參數 ，主要原因有三：
